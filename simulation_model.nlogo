; =============================================================================
; AGENT-BASED MODEL FOR MINIMAL/STRONG SOCIAL/MEDIA INFLUENCE (NON-CALIBRATED)
; =============================================================================
;
...
to setup-humans
  ...
  create-humans num-humans [
    ...
    ifelse random-float 100 < democrats% [
      set party "dem"
      set color blue
    ] [
      set party "rep"
...
to move-unhappy-humans
  ...
  if verbose? and any? movers [
    print (word count movers " agents relocated to more congenial patches.")
  ]
end
...
"" ""
PENS
"general" 1.0 0 -16777216 true "" "plot mean [affective-polarization] of humans"
"democrats" 1.0 0 -13345367 true "" "plot mean [affective-polarization] of humans with [party = \"dem\"]"
"republicans" 1.0 0 -2674135 true "" "plot mean [affective-polarization] of humans with [party = \"rep\"]"
...
"" ""
PENS
"Reps" 1.0 1 -5298144 true "" ";; Clear previous bars for THIS PEN (Republicans) only for this tick's redraw\nplot-pen-reset\n\n;; Calculate counts for Republicans in each ideology bin (1 through 5)\nlet rep-counts map [ideology-bin-value ->\n                   count humans with [party = \"rep\" and floor ideology = ideology-bin-value]\n                 ] (range 1 6 1)\n\n;; Plot each bar slightly to the RIGHT of the integer x-value for its category\nlet x-category 1\nlet x-offset 0.15   ;; This should be the positive counterpart of the Democrat offset\n\nforeach rep-counts [current-bin-count ->\n  plotxy (x-category + x-offset) current-bin-count\n  set x-category x-category + 1\n]"
"Dems" 1.0 1 -14070903 true "" ";; Clear previous bars for THIS PEN (Democrats) only for this tick's redraw\nplot-pen-reset\n\n;; Calculate counts for Democrats in each ideology bin (1 through 5)\nlet dem-counts map [ideology-bin-value ->\n                    count humans with [party = \"dem\" and floor ideology = ideology-bin-value]\n                  ] (range 1 6 1) ;; (range 1 6 1) creates the list [1 2 3 4 5]\n\n;; Plot each bar slightly to the LEFT of the integer x-value for its category\nlet x-category 1\nlet x-offset -0.15  ;; Adjust this offset carefully with the interval for good spacing\n                    ;; If interval is 0.3, this offset places the left edge of Dem bar at x_category - 0.3\n\nforeach dem-counts [current-bin-count ->\n  plotxy (x-category + x-offset) current-bin-count\n  set x-category x-category + 1\n]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot sd-ideology"
...
This non-calibrated version of model was developed as part of a study examining the mechanisms of affective polarization within digital and interpersonal communication environments. The authors thank the NetLogo community for ongoing support and modeling tools.
...
The model examines how four key mechanisms interact to shape polarization over time:
...
The model also includes a **backfire effect**, where extreme incongruence in information triggers increased polarization rather than moderation.

---

## HOW TO USE IT

1. Adjust global parameters (e.g., `global-social-influence`, `global-media-influence`, `global-selective-exposure`, `global-homogenous-discussion`) to set your desired scenario (see more clarification on the parameters in the technical appendix).
2. Press **Setup** to initialize agents and media patches on the grid.
3. Press **Go** to run the simulation. The model updates at each tick, simulating:
   - Interpersonal discussions
   - News consumption
   - Ideological updating
   - Affective polarization changes
   - Potential relocation if agents become ideologically uncomfortable
...
- Set `global-selective-exposure` to 0 and observe the effects of non-selective media consumption.
- Compare low vs. high values of `global-homogenous-discussion` to explore how discussion partner selection affects outcomes.
- Explore the influence of the **backfire effect** by varying the `p-backfire-when-incongruent` and `backfire-amp` parameters.
- Observe differences in **final polarization levels** under minimal vs. strong communication scenarios.
...
This model draws on theory and mechanisms from:
...
Simulation logic was inspired by agent-based frameworks in political communication, computational sociology, and sociophysics.
